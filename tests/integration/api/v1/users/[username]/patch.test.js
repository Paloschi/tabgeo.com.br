...
describe("PATCH /api/v1/users/[username]", () => {
  ...
  describe("Default user", () => {
    ...
    test("With duplicated `username`", async () => {
      ...
      const user2SessionObject =
        await orchestrator.createSession(activatedUser2);
    ...
    test("With `user2` targeting `user1`", async () => {
      ...
      const response = await fetch(
        `${webserver.origin}/api/v1/users/newuser1`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
    ...
    test("With unique `email`", async () => {
      ...
      expect(userInDatabase.email).toBe("uniqueEmail2@tabgeo.com.br");
    });
